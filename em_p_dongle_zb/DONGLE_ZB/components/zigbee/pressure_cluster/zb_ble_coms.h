...
void send_dev_defines (){
  apend_buff(SERIAL,sizeof(SERIAL));
  apend_buff(DATE,sizeof(DATE));
  apend_buff(HW_VERISON,sizeof(HW_VERISON));
  apend_buff(FW_VERSION,sizeof(FW_VERSION));
...
break;
  ...
  case 0xF5:
    if (request_buffer[1]==1){
      led_state(false,-1,true);
    }
    else {
    led_state(false,-1,false);
    }
  //toggle leds, sub 0 is off and 1 is on //make persistent// turn off leds auto
