...
#include "nrf_nvmc.h" //do not use it because of softdevice, instead, use nrf_soc/nrf_fstorage libraries
...
void write_nvmc (int value, int words, int address, int offset){
     //sd_flash_write((address + (offset*4)), &value, words);           //nrf_nvmc_write_words((address + (offset*4)), &value, words);
