...
void reset_the_interupt (uint8_t param){
    uart_enable = false;
    for(int i = 0; i<32; i++){request_buffer [i] = 0;}
    uart_request_i = 0;
    nrfx_gpiote_init();
  ...
  for (int i=0; i<80;i++){
    ...
    if (buffer_uart_array[i] == 'W' && buffer_uart_array[i+1] == 'W'){
      ...
      if (error_flag == false){
        ...
        Zigbee_uart.col_val = col_nvmc_update(float_checker(),(Zigbee_uart.cc_val*battery_def_int.cc_coef/1000000*c_dir));//TODO check this
...
uint8_t request_buffer [32] = {0};


bool mark_len = false;

bool uart_bms_case_check(){
  switch (buffer_uart_array[trans_len_i-9]){
    case 'S':
    case 'C':
      for (int i = 2; i<8; i++){
        if(buffer_uart_array[trans_len_i - i] != '0'){
          mark_len = true;
          break;
          }
       }
    ...
    default:
      if (mark_len == true && trans_len_i>60){
        mark_len = false;
        return true;
      } 
      if(mark_len == false && trans_len_i>60){
        return true;
        mark_len = false;
      }
  }
...
bool uart_request_case_check(){
  ...
  if (mark_len == true && trans_len_i>5){
    mark_len = false;
    return true;
  }
return false;
...
void get_uart (uint8_t param){
  ...
  do{
     ...
     if(uart_enable==false){
      ...
      request_buffer[trans_len_i] = cr_get;
     }
     trans_len_i++;
     ...
     if(cr_get == 0xD){
      if(uart_enable==false){
        uart_complete = uart_bms_case_check();
      }
      else {
        uart_complete = uart_request_case_check();
      }
     }
  }
  while (uart_complete == false);

  //reset some stuff
  led_state(false,1,false);
  send_len = trans_len_i;
  trans_len_i = 0;
  uart_complete = false;
